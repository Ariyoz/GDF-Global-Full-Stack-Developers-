        ...
        <div class="feed-posts">
          ...
          >
            ...
            <div class="post-content">
              ...
              <!-- Media (images/videos) for normal posts â€” shown BELOW text -->
        ...
        <div class="glass-card-static sidebar-card">
          ...
          <div v-if="trendingPosts.length" class="trending-posts-list">
            <div v-for="tp in trendingPosts.slice(0,5)" :key="tp.id" class="trending-post-item"
              ...
              <p class="tp-content">{{ tp.content?.slice(0, 80) }}{{ tp.content?.length > 80 ? 'â€¦' : '' }}</p>
              <div class="tp-stats">
...
const reactionEmojis = [] // Removed â€” using Twitter-style actions now
...
// Seed data is now in the store â€” no local posts array needed
...
<style scoped>
/* ROOT */
.feed-view { background: var(--background); min-height: 100vh; padding-top: 0.5rem; padding-bottom: 2rem; }

/* LAYOUT */
.feed-layout { display: block; padding: 0.75rem 0 2rem; }
...
/* LEFT SIDEBAR */
.sidebar-card { padding: 1.125rem; border-radius: var(--radius-xl); margin-bottom: 0.75rem; }
...
.sidebar-name { font-family: var(--font-headline); font-size: 0.875rem; font-weight: 700; color: var(--on-surface); }
.sidebar-role { font-size: 0.75rem; color: var(--on-surface-variant); margin-top: 1px; }
.sidebar-nav { display: flex; flex-direction: column; gap: 0.2rem; }
.sidebar-link {
  display: flex; align-items: center; gap: 0.625rem;
  padding: 0.6rem 0.75rem; border-radius: var(--radius-lg);
  font-family: var(--font-headline); font-size: 0.85rem; font-weight: 500;
  color: var(--on-surface-variant); text-decoration: none; transition: var(--transition-fast);
}
.sidebar-link:hover { background: var(--surface-container-low); color: var(--on-surface); }
.sidebar-link.active { background: rgba(99,14,212,0.08); color: var(--primary); font-weight: 600; }
[data-theme="dark"] .sidebar-link.active { background: rgba(168,85,247,0.12); }
...
.sidebar-section-title {
  ...
  color: var(--on-surface); display: flex; align-items: center; gap: 0.375rem; margin-bottom: 0.875rem;
}
.sidebar-empty { font-size: 0.8rem; color: var(--on-surface-variant); padding: 0.5rem 0; }

/* CREATE POST */
.create-post {
  background: var(--surface-container-lowest); border: 1px solid var(--outline-variant);
  border-radius: var(--radius-xl); padding: 1rem 1.125rem; margin-bottom: 0.5rem;
  box-shadow: var(--shadow-sm);
}
.create-post-top { display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.75rem; }
.post-avatar-sm {
  width: 36px; height: 36px; border-radius: 50%; flex-shrink: 0; overflow: hidden;
  background: var(--gradient-primary); color: #fff;
  font-family: var(--font-headline); font-size: 0.78rem; font-weight: 700;
  display: flex; align-items: center; justify-content: center;
}
.post-avatar-img { width: 100%; height: 100%; object-fit: cover; border-radius: 50%; }
.create-post-input {
  flex: 1; padding: 0.625rem 1rem; background: var(--surface-container-low);
  border: 1px solid var(--outline-variant); border-radius: var(--radius-full);
  font-family: var(--font-body); font-size: 0.875rem; color: var(--on-surface-variant);
  cursor: pointer; text-align: left; transition: var(--transition-fast);
}
.create-post-input:hover { border-color: var(--primary); background: var(--surface-container); }
.create-post-actions {
...
.post-action-type {
  ...
  padding: 0.4rem 0.875rem; background: var(--surface-container-low);
  border: none; border-radius: var(--radius-full);
  font-family: var(--font-headline); font-size: 0.78rem; font-weight: 600;
  color: var(--on-surface-variant); cursor: pointer; transition: var(--transition-fast);
}
.post-action-type:hover { background: var(--surface-container); color: var(--on-surface); }
.post-action-type .material-symbols-outlined { font-size: 16px; }

/* FEED TABS */
.feed-tabs {
  display: flex; border-bottom: 1px solid var(--outline-variant);
  margin-bottom: 0; background: var(--surface-container-lowest);
  border-radius: var(--radius-xl) var(--radius-xl) 0 0;
  overflow: hidden;
}
.feed-tab {
  flex: 1; padding: 0.875rem 1.25rem; background: none; border: none;
  border-bottom: 2px solid transparent; font-family: var(--font-headline);
  font-size: 0.875rem; font-weight: 500; color: var(--on-surface-variant);
  cursor: pointer; transition: var(--transition-fast);
}
.feed-tab:hover { color: var(--on-surface); background: var(--surface-container-low); }
.feed-tab.active { color: var(--primary); border-bottom-color: var(--primary); font-weight: 700; }

/* FEED POSTS */
.feed-posts { display: flex; flex-direction: column; gap: 0; }
.feed-post {
  padding: 1rem 1.25rem; border-bottom: 1px solid var(--outline-variant);
  background: var(--surface-container-lowest); transition: background 0.15s ease; cursor: default;
}
.feed-post:hover { background: var(--surface-container-low); }
.feed-post:last-child { border-bottom: none; }
.repost-label {
  display: flex; align-items: center; gap: 0.375rem;
  font-size: 0.78rem; color: var(--on-surface-variant);
  margin-bottom: 0.625rem; padding-left: 3rem;
}
.repost-label .material-symbols-outlined { font-size: 14px; color: #22c55e; }

/* POST HEADER */
.post-header { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 0.75rem; }
.post-author-info { display: flex; gap: 0.75rem; cursor: pointer; align-items: flex-start; flex: 1; min-width: 0; }
.post-author-info:hover .post-author-name { text-decoration: underline; }
.post-avatar {
  width: 40px; height: 40px; border-radius: 50%; flex-shrink: 0; overflow: hidden;
  background: var(--gradient-primary); color: #fff;
  font-family: var(--font-headline); font-size: 0.85rem; font-weight: 700;
  display: flex; align-items: center; justify-content: center;
}
.post-avatar-img { width: 100%; height: 100%; object-fit: cover; border-radius: 50%; }
...
.post-dropdown {
  ...
  min-width: 200px; padding: 0.375rem; overflow: hidden;
}
.dropdown-item {
  display: flex; align-items: center; gap: 0.625rem;
  padding: 0.625rem 0.875rem; width: 100%; background: none; border: none;
  border-radius: var(--radius-lg); font-family: var(--font-body); font-size: 0.875rem;
  color: var(--on-surface-variant); cursor: pointer; transition: var(--transition-fast); text-align: left;
}
.dropdown-item:hover { background: var(--surface-container-low); color: var(--on-surface); }
.dropdown-item--danger:hover { background: rgba(186,26,26,0.08); color: var(--error); }
.dropdown-item .material-symbols-outlined { font-size: 18px; }

/* POST CONTENT */
.post-content { margin-bottom: 0.875rem; padding-left: 3.25rem; }
.post-text-wrap { position: relative; }
.post-text {
  font-size: 0.9375rem; color: var(--on-surface); line-height: 1.65;
  white-space: pre-wrap; word-break: break-word;
}
.post-text.collapsed { display: -webkit-box; -webkit-line-clamp: 5; -webkit-box-orient: vertical; overflow: hidden; }
.read-more-btn {
  display: block; margin-top: 0.375rem; background: none; border: none;
  color: var(--primary); font-family: var(--font-headline); font-size: 0.8rem;
  font-weight: 600; cursor: pointer; padding: 0;
}
.post-media { display: flex; flex-wrap: wrap; gap: 0.5rem; margin-top: 0.875rem; }
.post-image { max-width: 100%; border-radius: var(--radius-xl); max-height: 500px; object-fit: cover; cursor: pointer; }
.post-video { width: 100%; border-radius: var(--radius-xl); max-height: 500px; }
.post-code-block { margin-top: 0.875rem; background: var(--surface-container); border-radius: var(--radius-xl); overflow: hidden; border: 1px solid var(--outline-variant); }
.code-header { padding: 0.5rem 1rem; background: var(--surface-container-high); border-bottom: 1px solid var(--outline-variant); }
.code-filename { font-family: var(--font-mono, monospace); font-size: 0.75rem; color: var(--on-surface-variant); }
.code-content { padding: 1rem; font-family: var(--font-mono, monospace); font-size: 0.8rem; color: var(--on-surface); overflow-x: auto; margin: 0; }
.post-link-preview { margin-top: 0.875rem; border: 1px solid var(--outline-variant); border-radius: var(--radius-xl); overflow: hidden; cursor: pointer; transition: border-color 0.15s; }
.post-link-preview:hover { border-color: var(--primary); }
.lp-image { width: 100%; max-height: 160px; object-fit: cover; }
.lp-body { padding: 0.875rem; }
.lp-site { display: block; font-size: 0.7rem; color: var(--on-surface-variant); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.2rem; }
.lp-title { display: block; font-family: var(--font-headline); font-size: 0.9rem; font-weight: 700; color: var(--on-surface); text-decoration: none; }
.lp-desc { display: block; font-size: 0.78rem; color: var(--on-surface-variant); margin-top: 0.2rem; }
.post-hashtags { display: flex; flex-wrap: wrap; gap: 0.375rem; margin-top: 0.5rem; }
.hashtag { color: var(--primary); font-size: 0.875rem; font-weight: 600; cursor: pointer; }
.hashtag:hover { text-decoration: underline; }
.post-link-inline { color: var(--primary); text-decoration: none; }
.post-link-inline:hover { text-decoration: underline; }
.post-document-link { display: inline-flex; align-items: center; gap: 0.5rem; color: var(--primary); text-decoration: none; font-size: 0.875rem; margin-top: 0.75rem; }

/* POST ACTIONS BAR */
.post-actions-bar {
  display: flex; align-items: center; gap: 0;
  padding-top: 0.5rem; margin-left: 3.25rem;
  border-top: 1px solid var(--outline-variant);
}
.action-btn {
  display: inline-flex; align-items: center; gap: 0.35rem;
...
.action-count { font-size: 0.8rem; }
.action-btn:hover { color: var(--primary); background: rgba(99,14,212,0.06); }
.like-btn:hover { color: #e91e63 !important; background: rgba(233,30,99,0.07) !important; }
.like-btn.active { color: #e91e63; }
.like-btn.active .material-symbols-outlined { font-variation-settings: 'FILL' 1; }
.repost-btn:hover { color: #22c55e !important; background: rgba(34,197,94,0.07) !important; }
.repost-btn.active { color: #22c55e; }
.bookmark-btn:hover { color: var(--primary) !important; background: rgba(99,14,212,0.07) !important; }
.bookmark-btn.active { color: var(--primary); }
.bookmark-btn.active .material-symbols-outlined { font-variation-settings: 'FILL' 1; }
.impressions-btn:hover { color: var(--on-surface) !important; background: var(--surface-container) !important; }
...
.comment-item { display: flex; gap: 0.625rem; align-items: flex-start; }
.comment-avatar {
...
.comment-header-line { display: flex; flex-wrap: wrap; align-items: center; gap: 0.35rem; margin-bottom: 0.2rem; }
.comment-author-name { font-family: var(--font-headline); font-size: 0.8rem; font-weight: 700; color: var(--on-surface); }
.comment-author-handle { font-size: 0.75rem; color: var(--on-surface-variant); }
.comment-time { font-size: 0.7rem; color: var(--on-surface-variant); }
.comment-text { font-size: 0.875rem; color: var(--on-surface); line-height: 1.5; }
.reply-mention { color: var(--primary); font-weight: 600; }
...
.comment-action--delete:hover { color: var(--error); }
.comment-replies { margin-top: 0.625rem; padding-left: 1rem; border-left: 2px solid var(--outline-variant); display: flex; flex-direction: column; gap: 0.625rem; }
.no-comments { font-size: 0.8rem; color: var(--on-surface-variant); text-align: center; padding: 1rem 0; }
.replying-to-bar {
  display: flex; align-items: center; justify-content: space-between;
  padding: 0.5rem 0.75rem; background: var(--surface-container);
  border-radius: var(--radius-lg); margin-bottom: 0.5rem;
  font-size: 0.8rem; color: var(--on-surface-variant);
}
.cancel-reply-btn { width: 20px; height: 20px; border-radius: 50%; background: transparent; border: none; display: flex; align-items: center; justify-content: center; color: var(--on-surface-variant); cursor: pointer; }
.cancel-reply-btn .material-symbols-outlined { font-size: 14px; }
.comment-input-row { display: flex; gap: 0.625rem; align-items: center; }
.comment-input {
  flex: 1; padding: 0.5rem 1rem; background: var(--surface-container);
  border: 1px solid var(--outline-variant); border-radius: var(--radius-full);
  font-family: var(--font-body); font-size: 0.875rem; color: var(--on-surface); outline: none;
  transition: var(--transition-fast);
}
.comment-input:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(99,14,212,0.1); }
.comment-send-btn {
...
.comment-send-btn .material-symbols-outlined { font-size: 16px; }

/* RIGHT SIDEBAR */
.trending-posts-list { display: flex; flex-direction: column; gap: 0.75rem; }
.trending-post-item {
  cursor: pointer; padding: 0.75rem; border-radius: var(--radius-lg);
  transition: background 0.15s ease; border: 1px solid transparent;
}
.trending-post-item:hover { background: var(--surface-container-low); border-color: var(--outline-variant); }
.tp-author { display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.3rem; }
...
.tp-name { font-family: var(--font-headline); font-size: 0.8rem; font-weight: 600; color: var(--on-surface); }
.tp-content { font-size: 0.8rem; color: var(--on-surface-variant); line-height: 1.45; margin-bottom: 0.3rem; }
.tp-stats { display: flex; gap: 0.75rem; font-size: 0.75rem; color: var(--on-surface-variant); }
.trending-skills { display: flex; flex-wrap: wrap; gap: 0.375rem; }
.skill-chip {
  padding: 0.3rem 0.75rem; background: var(--surface-container);
  border: 1px solid var(--outline-variant); border-radius: var(--radius-full);
  font-family: var(--font-headline); font-size: 0.78rem; font-weight: 600;
  color: var(--on-surface-variant); cursor: pointer; transition: var(--transition-fast);
}
.skill-chip:hover { border-color: var(--primary); color: var(--primary); background: rgba(99,14,212,0.05); }

/* LOADING & EMPTY */
.feed-loading { display: flex; flex-direction: column; gap: 0; }
.skeleton-post { padding: 1rem 1.25rem; border-bottom: 1px solid var(--outline-variant); }
.skeleton-header { display: flex; gap: 0.75rem; align-items: center; margin-bottom: 0.75rem; }
.skeleton-avatar { width: 40px; height: 40px; border-radius: 50%; flex-shrink: 0; }
.skeleton-text-group { flex: 1; display: flex; flex-direction: column; gap: 0.375rem; }
.skeleton-body { display: flex; flex-direction: column; gap: 0.375rem; }
.skeleton-line { border-radius: var(--radius-sm); height: 12px; }
.skeleton-shimmer {
...
.feed-empty {
  ...
  padding: 3rem 1rem; gap: 1rem; color: var(--on-surface-variant); font-size: 0.9rem;
}
.feed-end {
  text-align: center; padding: 1.5rem; color: var(--on-surface-variant);
  font-size: 0.85rem; border-top: 1px solid var(--outline-variant);
}
...
.compose-modal {
  width: 100%; max-width: 560px; background: var(--surface-container-lowest);
  border: 1px solid var(--outline-variant); border-radius: var(--radius-2xl);
...
.compose-body { padding: 1rem 1.25rem; }
.compose-user { display: flex; gap: 0.75rem; align-items: center; margin-bottom: 0.75rem; }
.compose-name { font-family: var(--font-headline); font-size: 0.875rem; font-weight: 700; color: var(--on-surface); }
.compose-textarea {
  width: 100%; min-height: 120px; background: var(--surface-container-lowest);
  border: none; outline: none; font-family: var(--font-body); font-size: 1rem;
  color: var(--on-surface); line-height: 1.65; resize: vertical;
}
.compose-textarea::placeholder { color: var(--on-surface-variant); opacity: 0.6; }
.compose-image-preview { padding: 0 1.25rem; display: flex; flex-direction: column; gap: 0.5rem; }
.compose-image-preview img { width: 100%; border-radius: var(--radius-xl); max-height: 300px; object-fit: cover; }
.compose-quote-preview { padding: 0 1.25rem; }
.quote-card { background: var(--surface-container); border: 1px solid var(--outline-variant); border-radius: var(--radius-xl); padding: 0.875rem; }
.quote-author { font-family: var(--font-headline); font-size: 0.8rem; font-weight: 700; color: var(--primary); margin-bottom: 0.25rem; }
.quote-text { font-size: 0.875rem; color: var(--on-surface-variant); }
.remove-image-btn { display: inline-flex; align-items: center; gap: 0.2rem; font-size: 0.78rem; color: var(--on-surface-variant); }
.compose-footer {
  display: flex; align-items: center; justify-content: space-between;
  padding: 0.875rem 1.25rem; border-top: 1px solid var(--outline-variant); gap: 0.5rem;
}
.compose-tools { display: flex; gap: 0.25rem; }
.image-option { display: inline-flex; align-items: center; gap: 0.3rem; font-size: 0.8rem; }
...
@media (max-width: 640px) {
  .feed-post { padding: 0.875rem 1rem; }
  .post-content { padding-left: 0; margin-top: 0.5rem; }
  .post-actions-bar { margin-left: 0; }
  .post-comments { padding-left: 0.5rem; }
